...
#   1. Node.js >= 18
#   2. Python 3 + pip
#   3. Applicable Python deps from requirements.txt
...
#   2   optional check failed (ADB or explicitly skipped tests), but everything else is fine
...
Exit codes:
  0   all required and optional checks pass
  1   a required check failed
  2   a non-fatal check (ADB or skipped tests) did not pass
EOF
}
...
PYTHON_BIN="python3"
[[ "$UNAME" == MINGW* || "$UNAME" == MSYS* || "$UNAME" == CYGWIN* ]] && PYTHON_BIN="python"
...
check_node() {
  if command -v node >/dev/null 2>&1; then
    ...
    if [[ "$major" -ge 18 ]]; then record "node" ok "v$major >= 18"; else record "node" fail "v$major < 18 — please upgrade"; fi
  else
...
check_python_deps() {
  ...
  local missing=() checked=0 skipped=0 pkg spec mod
  while IFS= read -r pkg; do
    [[ -z "$pkg" || "$pkg" =~ ^# ]] && continue
    spec="${pkg%%;*}"
    spec="$(printf '%s' "$spec" | xargs)"

    # PyGetWindow currently supplies the Windows-specific window helper path.
    # macOS/Linux use native AppleScript/wmctrl paths where implemented, so a
    # non-importable PyGetWindow must not make the whole install look broken.
    if [[ "$spec" == "pygetwindow" && "$UNAME" != MINGW* && "$UNAME" != MSYS* && "$UNAME" != CYGWIN* ]]; then
      skipped=$((skipped + 1))
      continue
    fi
  ...
  if [[ ${#missing[@]} -eq 0 ]]; then
    local detail="all $checked packages importable"
    [[ $skipped -gt 0 ]] && detail="$detail ($skipped platform-specific skipped)"
    record "python-deps" ok "$detail"
  else
...
check_adb() {
  if command -v adb >/dev/null 2>&1; then
    ...
    if [[ "$devices" -gt 0 ]]; then record "adb" ok "$devices device(s) connected"; else record "adb" warn "adb installed but no devices connected" true; fi
  else
...
check_server_status() {
  if (cd "$REPO_ROOT" && node src/server.js --status >/tmp/ndc-status.json 2>/tmp/ndc-status.err); then
    if node -e "const r = require('/tmp/ndc-status.json'); if (!r.desktop || !r.android) process.exit(1)" >/dev/null 2>&1; then
      local count; count="$(node -e "const r=require('/tmp/ndc-status.json'); console.log(Object.keys(r).filter(k => ['desktop','cua','android','codex'].includes(k) && r[k].available).length)")"
...
check_tests() {
  ...
  if (cd "$REPO_ROOT" && npm test --silent >/tmp/ndc-test.log 2>&1); then
    local pass; pass="$(grep -E '^# pass|^ℹ pass' /tmp/ndc-test.log | tail -1 | awk '{print $3}')"
    record "tests" ok "${pass:-unknown} tests pass"
  else
    record "tests" fail "npm test failed — see /tmp/ndc-test.log"
  fi
}
...
emit_json() {
  node -e '
    const rows = process.argv.slice(1).map((r) => {
      const [name, status, ...rest] = r.split("|");
      return { name, status, detail: rest.join("|") };
    });
    const required = rows.filter((r) => r.status === "fail");
    const optional = rows.filter((r) => r.status === "warn");
...
if [[ $JSON_OUT -eq 1 ]]; then
  ...
  if [[ $REQUIRED_FAIL -eq 0 && $OPTIONAL_FAIL -eq 0 ]]; then
    ...
    printf '\033[1;33m[check]\033[0m Required checks passed. %d optional check(s) did not pass.\n' "$OPTIONAL_FAIL"
  else
